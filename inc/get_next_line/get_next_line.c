...
/*   Updated: 2023/10/29 22:00:46 by mperetia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_new_static_buff(char *static_buff)
{
	...
	new_buff = (char *)malloc((ft_strlen_g(static_buff) - count1 + 1));
	if (!new_buff)
	{
		free(new_buff);
		return (NULL);
...
char	*ft_read_data(int fd, char *static_buff)
{
	...
	while (!ft_strchr_g(static_buff, '\n') && bytes_read != 0)
	{
		...
		buff[bytes_read] = '\0';
		static_buff = ft_strjoin_g(static_buff, buff);
	}
	free(buff);
	return (static_buff);
}
